...
public class Binary
{
	...
	public static Binary or(Binary num1, Binary num2) {
		...
		return new Binary(resultString);
	}
	...
	public static Binary multiply(Binary num1, Binary num2) {
		...
		// Iterate through the multiplier from right to left
		for (int i = multiplier.length() - 1; i >= 0; i--) {
			if (multiplier.charAt(i) == '1') {
				// If the current bit is 1, shift num1 by the correct amount and add to result
				String shiftedNum1 = num1.number;
				// Append zeros for shifting
				for (int j = 0; j < (multiplier.length() - 1 - i); j++) {
					shiftedNum1 += "0";
				}
